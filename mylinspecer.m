% function lineStyles = mylinspecer(N)
...
% looking colors.
% Bill Campbell 5/12/2014
% mylinspecer adds cyan at the low end and magenta at the high end for
% plotting outliers -- otherwise it's the same as linspecer.
% 
% lineStyles = mylinspecer(N);  makes N colors for you to use: lineStyles(ii,:)
% 
% colormap(mylinspecer); set your colormap to have easily distinguishable 
%                      colors and a pleasing aesthetic
% 
% lineStyles = mylinspecer(N,'qualitative'); forces the colors to all be distinguishable (up to 12)
% lineStyles = mylinspecer(N,'sequential'); forces the colors to vary along a spectrum 
% 
...
% figure; imagesc(A); colormap(mylinspecer); % mylinspecer colormap
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function lineStyles=mylinspecer(N,varargin)

if nargin==0 % return a colormap
    lineStyles = mylinspecer(64);
%     temp = [temp{:}];
...
switch N
    ...
    case {10, 11, 12}
        if qualFlag % force qualitative graphs
            ...
            lineStyles = cmap2linspecer(colorm(N+2));
        end
